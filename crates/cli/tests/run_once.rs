...
fn assert_timestamp_shape(timestamp: &str) {
    assert_eq!(timestamp.len(), 27, "expected YYYY-MM-DDThh:mm:ss.ffffffZ, got {timestamp:?}");
    assert!(timestamp.ends_with('Z'), "timestamps must be UTC: {timestamp:?}");
    let (date, time) = timestamp.split_once('T').expect("missing the T separator");
    assert_eq!(date.len(), 10, "{timestamp:?}");
    let fraction = time.split_once('.').expect("missing sub-second precision").1;
    assert_eq!(fraction.len(), 7, "expected 6 fractional digits and Z: {timestamp:?}");
    assert!(fraction[..6].chars().all(|c| c.is_ascii_digit()), "{timestamp:?}");
}
...
fn run_once_emits_one_well_formed_stats_event_to_stdout() {
    ...
    assert_timestamp_shape(event["timestamp"].as_str().expect("timestamp must be a string"));
    ...
    assert_eq!(stats["capture"]["enabled"], false, "capture lands in Phase 1");
    assert_eq!(stats["engine"]["enabled"], false, "detection lands in Phase 3");
}
...
fn the_same_event_reaches_the_file_sink() {
    ...
    assert_eq!(from_file, from_stdout, "both sinks must receive the identical event");
}
...
fn unparseable_rules_are_skipped_and_logged_with_a_reason() {
    ...
    assert!(stderr.contains("skipping unparseable rule"), "stderr:\n{stderr}");
    assert!(stderr.contains("test.rules:5"), "the log must name file and line:\n{stderr}");
    assert!(stderr.contains("test.rules:6"), "the log must name file and line:\n{stderr}");
...
fn diagnostics_never_contaminate_the_event_stream() {
    ...
    assert!(!String::from_utf8_lossy(&output.stderr).is_empty(), "diagnostics go to stderr");
}
...
fn the_sensor_id_persists_across_runs() {
    ...
    assert_eq!(id_of(&first), id_of(&second), "the sensor id must be stable across restarts");
...
fn a_missing_config_fails_with_a_clear_message_not_a_panic() {
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("definitely/not/here.yaml"), "stderr:\n{stderr}");
...
fn an_invalid_config_is_rejected_before_the_sensor_starts() {
    ...
    assert!(!output.status.success(), "a typo in the config must not start a half-configured sensor");
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("enabeld"), "the error must name the offending key:\n{stderr}");
